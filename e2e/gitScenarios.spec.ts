...
test('screening-highlights: the screeners\' highlights can be shown on the PDF', async () => {
  const { app } = await openScenario('screening-highlights')
  const page = await app.firstWindow()
  await page.getByRole('button', { name: 'Reviewer 1', exact: true }).click()
  await page.getByText('Searching code with natural language').first().click()
  const box = page.getByLabel(/Show markings from screening \(2\)/)
  await expect(box).toBeVisible()
  await expect(page.locator('.pdf-mark-rect.is-foreign')).toHaveCount(0)
  await box.check()
  await expect(page.locator('.pdf-mark-rect.is-foreign')).toHaveCount(1)
