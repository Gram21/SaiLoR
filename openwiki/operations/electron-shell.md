...
- A file that isn't valid JSON is parsed defensively — skipped, not thrown over, so one corrupt tree cannot block opening a project — but its `<paperId>/<file>` path is collected in `loadPaperFiles`' `corruptOut` array, which `project:open`/`project:openPath` pass down and return as `corrupt` alongside `{ path, text }`. `ElectronAdapter` forwards it as `OpenedProject.corruptFiles`, and `openProject`/`openRecent` in `src/state/store.ts` turn a non-empty list into a `loadError` (`corruptFilesWarning`, listing up to 10 paths) shown by `ErrorPanel`. Without that banner the skip is invisible: the seat simply looks unannotated, and a reviewer would redo or overwrite work that is still on disk.

Writing goes through `writeProjectFiles` (shared by `project:save` and every git handler that needs to put a specific project state onto disk — `git:commitPartial`'s write→add→commit→restore swap and `git:pullFinish`'s merge result both go through it, so "how a split project is written" has exactly one implementation). It writes `project.json` and reconciles the whole `annotations/` folder against the `files` list: a non-null entry is written, a `null` entry is deleted — unless the file already there fails `isDeletableAnnotationText` (`src/model/project.ts`), i.e. is non-empty and not valid JSON, in which case it is kept: `loadPaperFiles` reports an unparseable annotation file as absent so one corrupt tree cannot block opening a project, and without this guard the next save (or autosave, commit, pull-finish, branch-switch) would delete a file whose only defect was git conflict markers. Every annotation target is containment-checked twice (string and real-path), and `assertNotSymlink` refuses to write through a symlink — a project folder can arrive by zip/USB/shared drive with a symlink already in it, and `writeFile` follows one, so a `<name>-fulltext.json` shipped as a symlink to `~/.zshrc` turned one click into an overwrite of a startup file.
