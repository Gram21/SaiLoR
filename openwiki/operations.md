...
`PlatformAdapter` implementation is now `createUnsupportedAdapter` (`src/platform/unsupported.ts`),
which answers the two reads that genuinely happen before `App`'s `isElectron()` gate (`kind`,
`getRecents`) for real and throws `"SaiLoR for the web is discontinued — use the desktop app."` on
every other method via a `Proxy` trap, as a backstop for the handful of reads (`getRecents()`) that
happen at store module load, before `App` ever renders and can show the gate.
...
Tests live in `src/**/*.test.{ts,tsx}`. `src/model/model.test.ts` covers the model layer, `src/state/store.test.ts` covers the store's undo/redo history (field-edit coalescing, add/remove undo, redo-stack clearing), and `src/state/editorStore.test.ts` covers the project editor (schema ⇄ on-disk conversion and round-trip, drag-and-drop tree moves incl. the self/descendant guard, draft validation, and that `buildProjectJson` produces a file the real `loadProject` accepts with annotations preserved) — split into sibling files for the platform-stubbed flows (`editorStore.pdfs.test.ts` for `addPdfs`/`addPdfFolder` deduplication, `editorStore.import.test.ts` for `importReferences` matching/merging and the "just added" marks, `editorStore.history.test.ts` for undo/redo). `src/state/store.marks.test.ts` covers PDF mark mutations and their undo steps (`addHighlight`/`setMarkComment`/`setMarkColor`/`removeMark`, grouped-mark propagation, comment coalescing). `src/state/store.readingPosition.test.ts` covers the per-project reading-position persistence (restore on reopen, `offsetFraction` defaulting to `0` for older stored values, clamping a malformed fraction, the no-handle and gone-paper cases). `src/git/concurrentRead.test.ts` covers `readAllConcurrently`'s out-of-order resolution and concurrency; `src/git/deriveGitInfo.test.ts` covers the field-to-result mapping. `src/model/refPreview.test.ts` covers `detectEntryBox`'s entry-fitting against reference-list, two-column, and sparse-page layouts. `src/model/references.test.ts` pins the BibTeX/RIS/CSL-JSON parser adversarially (nested braces, missing fields, CRLF, a BOM, garbage input, an entry with no title) — every case must resolve to *some* array, never a thrown error. Vitest is configured with jsdom environment and global test APIs (describe/it/expect available without import, though the test files import them explicitly).
...
| `pdfReadingPosition.integration.test.tsx` | "Continue where you left off" restore: reopening a project scrolls the PDF to the remembered page — re-snapping as earlier pages grow from placeholder to real height rather than landing where the placeholder sat — and restores the scroll *offset within* the page via the rect-delta technique. Asserts directly on `scrollTop` against a hand-modeled geometry. |

Two deliberate simplifications recur across these fakes: (1) the scratch repos stay in the **legacy single-file project shape** (`papers[].annotations` inline) throughout, rather than the split `project.json` + `annotations/<id>/*.json` layout a real save produces — sufficient for exercising the git logic itself, and any test that needs the split shape reassembles it via the real `assembleLegacyProjectJson` before writing, so `git status`/`diff` never sees a spurious format-only change; (2) jsdom performs no real layout, so `annotationWorkflow`'s PDF-selection test patches `Element.prototype.getBoundingClientRect`/`Range.prototype.getClientRects` to fixed, non-zero rects — `react-pdf` itself is mocked to skip real PDF.js parsing/canvas rendering, down to the exact DOM shape (`.react-pdf__Page`, `.react-pdf__Page__textContent`) `PdfViewer`'s own selection code queries for. The reading-position integration test deliberately does *not* use fixed rects: its `getBoundingClientRect` mock behaves like a real scrollable container (a page's `top` reflects its `.pdf-scroll` ancestor's current `scrollTop`), because the bug it pins — a self-correcting `scrollTop +=` re-applied across a placeholder→real-height settling window — could not be told from one that silently composes wrong without that scroll feedback.
...
Once gated, `build` fans out over a matrix of `macos-latest`, `windows-latest`, and `ubuntu-latest`, runs `./scripts/build-electron.sh` on each, and then attaches the OS-specific installer plus the `electron-updater` metadata to the release via `softprops/action-gh-release`: `release/*.dmg` on mac, `release/*.exe` + `release/*.exe.blockmap` + `release/latest.yml` + `release/latest.yml.sig` on Windows, and `release/*.AppImage` + `release/latest-linux.yml` + `release/latest-linux.yml.sig` on Linux. The Windows/Linux extras are the update feed `electron-updater` downloads at runtime (see the `--publish never` note above), so omitting them from a release breaks the in-app self-updater for that platform; macOS has no equivalent. Each job also uploads its artifact to the workflow run (`actions/upload-artifact`), so a manual `workflow_dispatch` run — which has no release to attach to — still produces downloadable installers. The builds are unsigned (no code-signing certificates are configured). As with CI, the build logic lives in the shell script so it stays runnable locally and portable across providers.

**Signed update feed (Windows/Linux).** On Windows and Linux a "Sign update metadata" step runs `scripts/sign-release.cjs` against the just-built `latest.yml`/`latest-linux.yml` with the `RELEASE_SIGNING_PRIVATE_KEY` secret (an Ed25519 PEM private key), writing `<feed>.sig` next to it, which the matrix then attaches alongside the feed. The installer itself remains unsigned (no Authenticode certificate), but the app verifies the feed's signature against a public key baked in at build time (`src/model/updateSignature.ts`, imported by `electron/main.ts`'s `update:download` handler) before ever downloading an update — so a hostile release published under a compromised token is caught even without a purchased certificate. What a real Authenticode certificate would still add is removing Windows SmartScreen's "unrecognized publisher" warning on the installer, which only builds reputation through an identity over time. macOS has no `electron-updater` feed at all and no signing step.
...
Undo and redo operate on annotation changes and PDF mark mutations. The store keeps session-only history snapshots in `src/state/store.ts`, and consecutive edits to the same field collapse into a single undo step instead of one step per keystroke. Add/remove instance actions and PDF mark mutations (`addHighlight`, `setMarkComment`, `setMarkColor`, `removeMark`, `linkMarkToField`, `unlinkMarkFromField`) also create their own history entries — `setMarkComment` coalesces consecutive keystrokes into one step, the same way a field value does.
