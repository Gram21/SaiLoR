...
| `visibleIf` | no | — | Name of a sibling field (same `children` array, or the same root-level list) that gates this node's visibility: hidden until that sibling is "answered" — `true` for a boolean, non-null/non-empty for anything else. An invalid reference (self, a nonexistent name, or a group with no `type`) is silently dropped during resolution rather than rejected, including one left stale by a later rename/removal of the target sibling — same degrade-defensively convention as `required` on a boolean above |
