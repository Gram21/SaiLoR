...
| `visibleIf` | no | — | Name of a field that gates this node's visibility: hidden until that field is "answered" — `true` for a boolean, non-null/non-empty for anything else. Either a same-level sibling (same `children` array, or the same root-level list) or any field along this node's direct ancestor chain (parent, parent's parent, ...) — never a cousin (an ancestor's own sibling), only a straight lineage. An invalid reference (self, a nonexistent name, or a group with no `type`) is silently dropped during resolution rather than rejected, including one left stale by a later rename/removal of the target field — same degrade-defensively convention as `required` on a boolean above |
