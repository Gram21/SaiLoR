...
/** The wording that differs between a git merge and a save that met changes on disk. */
export interface ConflictDialogLabels {
  title: string
  intro: string
  /** The other side's value as a noun phrase, e.g. "the remote value". */
  theirsValue: string
  useAllTheirs: string
  cancel: string
  cancelTitle: string
...
interface Props {
  merge: ConflictResolutionState
  labels: ConflictDialogLabels
  error: string | null
  onResolve: (id: string, value: FieldValue) => void
  onTakeAll: (side: 'ours' | 'theirs', ids: string[]) => void
  onFinish: () => void
  onCancel: () => void
  onDismissError: () => void
}
...
/**
 * The conflict resolution list for a three-way merge of the project. Everything
 * a field-level merge could settle on its own is already merged by the time
 * this opens; every row here is a field **both** sides changed, to different
 * things, which is the one case nothing but a person can settle.
 *
 * Deliberately has no Escape, no backdrop-click, and no × in the header: a
 * git merge is in progress for as long as this is open, and dismissing it
 * without finishing or explicitly cancelling would leave the checkout in a
 * state the reviewer cannot get out of without the command line. Only the two
 * buttons at the bottom leave.
 */
    ...
    <div className="modal-overlay">
      ...
      >
        <div className="modal-head">
          <strong>
            {labels.title} <span className="git-merge-progress">{decidedCount} of {merge.conflicts.length} decided</span>
          </strong>
        </div>
        ...
        <div className="modal-body">
          ...
          <div className="git-merge-bulk">
            <button
              ...
              onClick={() => onTakeAll('ours', bulkEligibleIds)}
            >
              Use all mine
            </button>
            <button
              type="button"
              title={`Use ${labels.theirsValue} for every undecided field below, except another reviewer's own answers`}
              onClick={() => onTakeAll('theirs', bulkEligibleIds)}
            >
              {labels.useAllTheirs}
            </button>
          </div>
                    ...
                    <ul className="git-merge-group-rows">
                      {g.conflicts.map((c) => (
                        <ConflictRow
                          ...
                          onTake={(side) => onResolve(c.id, side === 'ours' ? c.ours : c.theirs)}
                          onChange={(v) => onResolve(c.id, v)}
                        />
                      ))}
                    </ul>
                  )}
        ...
        {/* A failure raised by finishing (a rejected merge commit, a refused
            save) would otherwise leave an unexplained dialog whose Finish
            button silently re-fails. */}
        {error && (
        ...
        <div className="git-merge-footer">
          ...
          <button
            ...
            className="primary"
            disabled={!allDecided}
            title={allDecided ? labels.finishTitle : 'Decide every conflict above before finishing'}
            onClick={onFinish}
          >
            {labels.finish}
          </button>
...
interface ConflictRowProps {
  ...
  foreign: boolean
  value: FieldValue
  /** How the other side is named, e.g. "the remote value". */
  theirsValue: string
