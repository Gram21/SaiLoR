...
function openDb(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    ...
    let settled = false
    req.onsuccess = () => {
      // The blocked path below may already have rejected. If the blocking tab
      // then closes, the open still succeeds — and without this the connection
      // would be opened and never closed, because the caller's `finally` ran
      // long ago against a promise that had already settled.
      if (settled) {
        req.result.close()
        return
      }
